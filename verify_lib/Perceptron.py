...
for epoch in range(epoch_count):
    ...
    for sample in range(training_count):
        ...
        for node in range(H_dim):
            ...
            preActivation_H[node] = np.dot(
                training_data[sample, :],
                weights_ItoH[:, node]
            )

            postActivation_H[node] = logistic(
                preActivation_H[node]
            )


        preActivation_O = np.dot(
            postActivation_H,
            weights_HtoO
        )
        ...
        for H_node in range(H_dim):
            ...
            gradient_HtoO = (
                S_error * postActivation_H[H_node]
            )


            for I_node in range(I_dim):

                input_value = training_data[
                    sample, I_node
                ]
...
print(
    correct_classification_count * 100 / validation_count
)
